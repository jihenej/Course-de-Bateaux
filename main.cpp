...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(width, height), "Jeu de Bateau");
    window.setFramerateLimit(60);

    // 2. Initialisation des différentes classes
    Menu myMenu(width, height);
    SettingsScreen mySettings(width, height);
    GameScreen myGameScreen(width, height, nb_col);
    ...
    while (window.isOpen()) {
        ...
        if (currentState == GameState::Playing) {
            ...
            myGameScreen.UpdateGame(deltaTime,width,height,scale);
        }
